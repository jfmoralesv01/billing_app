CLASS zsac_cl_load_billdata_j DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zsac_cl_load_billdata_j IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA lt_bill_header TYPE STANDARD TABLE OF zsac_bill_head_j.

    DELETE FROM zsac_bill_head_j.
    ...
    INSERT zsac_bill_head_j FROM TABLE @lt_bill_header.
    out->write( 'Billing data is inserted' ).

    DATA lt_bill_item TYPE STANDARD TABLE OF zsac_bill_item_j.

    DELETE FROM zsac_bill_item_j.
    ...
    INSERT zsac_bill_item_j FROM TABLE @lt_bill_item.
